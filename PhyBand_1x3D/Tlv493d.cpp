...
void Tlv493d::begin(int SenNo) {
  if (SenNo==0)
    begin(Wire, TLV493D_ADDRESS1, true);
  else if (SenNo==1)
    begin(Wire, TLV493D_ADDRESS2, false);
}

void Tlv493d::begin(int SenNo, TwoWire &bus) {
  if (SenNo==0)	
    begin(bus, TLV493D_ADDRESS1, true);
  else if (SenNo==1)
    begin(bus, TLV493D_ADDRESS2, false);
}

void Tlv493d::begin(TwoWire &bus, Tlv493d_Address_t slaveAddress, bool reset) {
  if (slaveAddress==0x5E) {
    pinMode(LED2, OUTPUT);	
    digitalWrite(LED2, HIGH);
    delay(50);
  }
  else if (slaveAddress==0x1F) {
...
float Tlv493d::getX(void) {
	return static_cast<float>(mXdata) * TLV493D_BX_MULT+TLV493D_BX_OFFSET;
}


float Tlv493d::getY(void) {
	return static_cast<float>(mYdata) * TLV493D_BY_MULT+TLV493D_BY_OFFSET;
}


float Tlv493d::getZ(void) {
	return static_cast<float>(mZdata) * TLV493D_BZ_MULT+TLV493D_BZ_MULT;
}
